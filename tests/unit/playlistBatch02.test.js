...
const BATCH = [
  { sessionId: '02A', videoId: '_mEd_2G8glg', title: '第2講 如何探究真相（上）' },
  { sessionId: '02B', videoId: '__8WjGF3hhw', title: '第2講 如何探究真相（下）' },
];

test('M4 batch-02 registers session 02A and 02B in course and catalog', () => {
...
test('M4 batch-02 session JSONs carry timestamped transcript and question index', () => {
  for (const item of BATCH) {
...
test('M4 batch-02 audio map records youtube provenance without claiming a local proxy', () => {
...
test('M4 batch-02 toc indexes batch questions per session', () => {
