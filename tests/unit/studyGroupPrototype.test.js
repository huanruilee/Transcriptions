...
test('prototype-33 preserves source identity and passes the machine ASR gate', () => {
  const evidence = path.join(ROOT, 'reviews/evidence/study-group-2025/prototype-33');
  const candidate = JSON.parse(fs.readFileSync(path.join(evidence, 'candidate.json'), 'utf8'));
  const raw = JSON.parse(fs.readFileSync(path.join(evidence, 'raw_asr.json'), 'utf8'));
  const manifest = JSON.parse(fs.readFileSync(path.join(evidence, 'run_manifest.json'), 'utf8'));
  const review = fs.readFileSync(path.join(evidence, 'review.md'), 'utf8');
  assert.equal(candidate.source.videoId, '6X3ygKQVK6w');
  assert.equal(candidate.source.playlistIndex, 33);
  assert.equal(candidate.segments.length, 4129);
  assert.equal(candidate.segments.length, raw.segments.length);
  assert.equal(candidate.questionIndex.length, 0);
  assert.equal(candidate.teacherSummaries.length, 0);
  assert.equal(candidate.provenance.temporaryAudioDeleted, true);
  assert.equal(manifest.videoId, '6X3ygKQVK6w');
  assert.equal(manifest.status, 'ASR_OK');
  assert.equal(manifest.checks.audioCoveragePct, 99.1918);
  assert.equal(manifest.checks.repeatedCharacterSegments, 0);
  assert.equal(raw.segments.filter((segment) => /(.)\1{9,}/u.test(segment.text)).length, 0);
  assert.equal(manifest.cleanup.temporaryAudioDeleted, true);
  assert.equal(manifest.cleanup.disposableEnvironmentDeleted, true);
  assert.match(manifest.artifacts.rawAsrPath, /^reviews\/evidence\/study-group-2025\/prototype-33\//);
  assert.match(manifest.artifacts.candidatePath, /^reviews\/evidence\/study-group-2025\/prototype-33\//);
  assert.match(review, /Status:\s+\*\*BLOCKED\*\*/);
  for (let index = 0; index < raw.segments.length; index += 1) {
    assert.equal(candidate.segments[index].id, raw.segments[index].id);
    assert.equal(candidate.segments[index].start, raw.segments[index].start);
...
test('prototype-34 preserves source identity and passes the machine ASR gate', () => {
  const evidence = path.join(ROOT, 'reviews/evidence/study-group-2025/prototype-34');
  const candidate = JSON.parse(fs.readFileSync(path.join(evidence, 'candidate.json'), 'utf8'));
  const raw = JSON.parse(fs.readFileSync(path.join(evidence, 'raw_asr.json'), 'utf8'));
  const manifest = JSON.parse(fs.readFileSync(path.join(evidence, 'run_manifest.json'), 'utf8'));
  const review = fs.readFileSync(path.join(evidence, 'review.md'), 'utf8');
  assert.equal(candidate.source.videoId, '-m6sV_rS1bU');
  assert.equal(candidate.source.playlistIndex, 34);
  assert.equal(candidate.segments.length, 3573);
  assert.equal(candidate.segments.length, raw.segments.length);
  assert.equal(candidate.questionIndex.length, 0);
  assert.equal(candidate.teacherSummaries.length, 0);
  assert.equal(candidate.provenance.temporaryAudioDeleted, true);
  assert.equal(manifest.videoId, '-m6sV_rS1bU');
  assert.equal(manifest.status, 'ASR_OK');
  assert.equal(manifest.checks.audioCoveragePct, 98.5471);
  assert.equal(manifest.checks.repeatedCharacterSegments, 0);
  assert.equal(raw.segments.filter((segment) => /(.)\1{9,}/u.test(segment.text)).length, 0);
  assert.equal(manifest.cleanup.temporaryAudioDeleted, true);
  assert.equal(manifest.cleanup.disposableEnvironmentDeleted, true);
  assert.match(manifest.artifacts.rawAsrPath, /^reviews\/evidence\/study-group-2025\/prototype-34\//);
  assert.match(manifest.artifacts.candidatePath, /^reviews\/evidence\/study-group-2025\/prototype-34\//);
  assert.match(review, /Status:\s+\*\*BLOCKED\*\*/);
  for (let index = 0; index < raw.segments.length; index += 1) {
    assert.equal(candidate.segments[index].id, raw.segments[index].id);
    assert.equal(candidate.segments[index].start, raw.segments[index].start);
...
test('prototype-35 preserves source identity and passes the machine ASR gate', () => {
  const evidence = path.join(ROOT, 'reviews/evidence/study-group-2025/prototype-35');
  const candidate = JSON.parse(fs.readFileSync(path.join(evidence, 'candidate.json'), 'utf8'));
  const raw = JSON.parse(fs.readFileSync(path.join(evidence, 'raw_asr.json'), 'utf8'));
  const manifest = JSON.parse(fs.readFileSync(path.join(evidence, 'run_manifest.json'), 'utf8'));
  const review = fs.readFileSync(path.join(evidence, 'review.md'), 'utf8');
  assert.equal(candidate.source.videoId, 'Kfrn27RgpXo');
  assert.equal(candidate.source.playlistIndex, 35);
  assert.equal(candidate.segments.length, 4198);
  assert.equal(candidate.segments.length, raw.segments.length);
  assert.equal(candidate.questionIndex.length, 0);
  assert.equal(candidate.teacherSummaries.length, 0);
  assert.equal(candidate.provenance.temporaryAudioDeleted, true);
  assert.equal(manifest.videoId, 'Kfrn27RgpXo');
  assert.equal(manifest.status, 'ASR_OK');
  assert.equal(manifest.checks.audioCoveragePct, 98.1041);
  assert.equal(manifest.checks.repeatedCharacterSegments, 0);
  assert.equal(raw.segments.filter((segment) => /(.)\1{9,}/u.test(segment.text)).length, 0);
  assert.equal(manifest.cleanup.temporaryAudioDeleted, true);
  assert.equal(manifest.cleanup.disposableEnvironmentDeleted, true);
  assert.match(manifest.artifacts.rawAsrPath, /^reviews\/evidence\/study-group-2025\/prototype-35\//);
  assert.match(manifest.artifacts.candidatePath, /^reviews\/evidence\/study-group-2025\/prototype-35\//);
  assert.match(review, /Status:\s+\*\*BLOCKED\*\*/);
  for (let index = 0; index < raw.segments.length; index += 1) {
    assert.equal(candidate.segments[index].id, raw.segments[index].id);
    assert.equal(candidate.segments[index].start, raw.segments[index].start);
...
test('prototype-36 preserves source identity and passes the machine ASR gate', () => {
  const evidence = path.join(ROOT, 'reviews/evidence/study-group-2025/prototype-36');
  const candidate = JSON.parse(fs.readFileSync(path.join(evidence, 'candidate.json'), 'utf8'));
  const raw = JSON.parse(fs.readFileSync(path.join(evidence, 'raw_asr.json'), 'utf8'));
  const manifest = JSON.parse(fs.readFileSync(path.join(evidence, 'run_manifest.json'), 'utf8'));
  const review = fs.readFileSync(path.join(evidence, 'review.md'), 'utf8');
  assert.equal(candidate.source.videoId, 'AA6JyKOI1AA');
  assert.equal(candidate.source.playlistIndex, 36);
  assert.equal(candidate.segments.length, 4127);
  assert.equal(candidate.segments.length, raw.segments.length);
  assert.equal(candidate.questionIndex.length, 0);
  assert.equal(candidate.teacherSummaries.length, 0);
  assert.equal(candidate.provenance.temporaryAudioDeleted, true);
  assert.equal(manifest.videoId, 'AA6JyKOI1AA');
  assert.equal(manifest.status, 'ASR_OK');
  assert.equal(manifest.checks.audioCoveragePct, 98.7128);
  assert.equal(manifest.checks.repeatedCharacterSegments, 0);
  assert.equal(raw.segments.filter((segment) => /(.)\1{9,}/u.test(segment.text)).length, 0);
  assert.equal(manifest.cleanup.temporaryAudioDeleted, true);
  assert.equal(manifest.cleanup.disposableEnvironmentDeleted, true);
  assert.match(manifest.artifacts.rawAsrPath, /^reviews\/evidence\/study-group-2025\/prototype-36\//);
  assert.match(manifest.artifacts.candidatePath, /^reviews\/evidence\/study-group-2025\/prototype-36\//);
  assert.match(review, /Status:\s+\*\*BLOCKED\*\*/);
  for (let index = 0; index < raw.segments.length; index += 1) {
    assert.equal(candidate.segments[index].id, raw.segments[index].id);
    assert.equal(candidate.segments[index].start, raw.segments[index].start);
...
test('prototype-37 preserves source identity and passes the machine ASR gate', () => {
  const evidence = path.join(ROOT, 'reviews/evidence/study-group-2025/prototype-37');
  const candidate = JSON.parse(fs.readFileSync(path.join(evidence, 'candidate.json'), 'utf8'));
  const raw = JSON.parse(fs.readFileSync(path.join(evidence, 'raw_asr.json'), 'utf8'));
  const manifest = JSON.parse(fs.readFileSync(path.join(evidence, 'run_manifest.json'), 'utf8'));
  const review = fs.readFileSync(path.join(evidence, 'review.md'), 'utf8');
  assert.equal(candidate.source.videoId, 'DbBo5ZedMVg');
  assert.equal(candidate.source.playlistIndex, 37);
  assert.equal(candidate.segments.length, 4109);
  assert.equal(candidate.segments.length, raw.segments.length);
  assert.equal(candidate.questionIndex.length, 0);
  assert.equal(candidate.teacherSummaries.length, 0);
  assert.equal(candidate.provenance.temporaryAudioDeleted, true);
  assert.equal(manifest.videoId, 'DbBo5ZedMVg');
  assert.equal(manifest.status, 'ASR_OK');
  assert.equal(manifest.checks.audioCoveragePct, 99.1286);
  assert.equal(manifest.checks.repeatedCharacterSegments, 0);
  assert.equal(raw.segments.filter((segment) => /(.)\1{9,}/u.test(segment.text)).length, 0);
  assert.equal(manifest.cleanup.temporaryAudioDeleted, true);
  assert.equal(manifest.cleanup.disposableEnvironmentDeleted, true);
  assert.match(manifest.artifacts.rawAsrPath, /^reviews\/evidence\/study-group-2025\/prototype-37\//);
  assert.match(manifest.artifacts.candidatePath, /^reviews\/evidence\/study-group-2025\/prototype-37\//);
  assert.match(review, /Status:\s+\*\*BLOCKED\*\*/);
  for (let index = 0; index < raw.segments.length; index += 1) {
    assert.equal(candidate.segments[index].id, raw.segments[index].id);
    assert.equal(candidate.segments[index].start, raw.segments[index].start);
...
test('prototype-39 preserves source identity and passes the machine ASR gate', () => {
  const evidence = path.join(ROOT, 'reviews/evidence/study-group-2025/prototype-39');
  const candidate = JSON.parse(fs.readFileSync(path.join(evidence, 'candidate.json'), 'utf8'));
  const raw = JSON.parse(fs.readFileSync(path.join(evidence, 'raw_asr.json'), 'utf8'));
  const manifest = JSON.parse(fs.readFileSync(path.join(evidence, 'run_manifest.json'), 'utf8'));
  const review = fs.readFileSync(path.join(evidence, 'review.md'), 'utf8');
  assert.equal(candidate.source.videoId, 'lu1dedE3Ncs');
  assert.equal(candidate.source.playlistIndex, 39);
  assert.equal(candidate.segments.length, 4179);
  assert.equal(candidate.segments.length, raw.segments.length);
  assert.equal(candidate.questionIndex.length, 0);
  assert.equal(candidate.teacherSummaries.length, 0);
  assert.equal(candidate.provenance.temporaryAudioDeleted, true);
  assert.equal(manifest.videoId, 'lu1dedE3Ncs');
  assert.equal(manifest.status, 'ASR_OK');
  assert.equal(manifest.checks.audioCoveragePct, 98.3484);
  assert.equal(manifest.checks.repeatedCharacterSegments, 0);
  assert.equal(raw.segments.filter((segment) => /(.)\1{9,}/u.test(segment.text)).length, 0);
  assert.equal(manifest.cleanup.temporaryAudioDeleted, true);
  assert.equal(manifest.cleanup.disposableEnvironmentDeleted, true);
  assert.match(manifest.artifacts.rawAsrPath, /^reviews\/evidence\/study-group-2025\/prototype-39\//);
  assert.match(manifest.artifacts.candidatePath, /^reviews\/evidence\/study-group-2025\/prototype-39\//);
  assert.match(review, /Status:\s+\*\*BLOCKED\*\*/);
  for (let index = 0; index < raw.segments.length; index += 1) {
    assert.equal(candidate.segments[index].id, raw.segments[index].id);
    assert.equal(candidate.segments[index].start, raw.segments[index].start);
...
test('prototype-41 preserves source identity and passes the machine ASR gate', () => {
  const evidence = path.join(ROOT, 'reviews/evidence/study-group-2025/prototype-41');
  const candidate = JSON.parse(fs.readFileSync(path.join(evidence, 'candidate.json'), 'utf8'));
  const raw = JSON.parse(fs.readFileSync(path.join(evidence, 'raw_asr.json'), 'utf8'));
  const manifest = JSON.parse(fs.readFileSync(path.join(evidence, 'run_manifest.json'), 'utf8'));
  const review = fs.readFileSync(path.join(evidence, 'review.md'), 'utf8');
  assert.equal(candidate.source.videoId, 'AtYqSdUIXDo');
  assert.equal(candidate.source.playlistIndex, 41);
  assert.equal(candidate.segments.length, 3984);
  assert.equal(candidate.segments.length, raw.segments.length);
  assert.equal(candidate.questionIndex.length, 0);
  assert.equal(candidate.teacherSummaries.length, 0);
  assert.equal(candidate.exclusions[0].reason, 'speaker role is not proven');
  assert.equal(candidate.provenance.temporaryAudioDeleted, true);
  assert.equal(manifest.videoId, 'AtYqSdUIXDo');
  assert.equal(manifest.status, 'ASR_OK');
  assert.equal(manifest.checks.audioCoveragePct, 98.9082);
  assert.equal(manifest.checks.totalWordTokens, 0);
  assert.equal(manifest.checks.repeatedCharacterSegments, 0);
  assert.equal(raw.segments.filter((segment) => /(.)\1{9,}/u.test(segment.text)).length, 0);
  assert.equal(manifest.cleanup.temporaryAudioDeleted, true);
  assert.equal(manifest.cleanup.disposableEnvironmentDeleted, true);
  assert.deepEqual(manifest.cleanup.removedPaths, ['audio/source.mp4', 'audio']);
  assert.match(manifest.artifacts.rawAsrPath, /^reviews\/evidence\/study-group-2025\/prototype-41\//);
  assert.match(manifest.artifacts.candidatePath, /^reviews\/evidence\/study-group-2025\/prototype-41\//);
  assert.match(review, /Status:\s+\*\*BLOCKED\*\*/);
  for (let index = 0; index < raw.segments.length; index += 1) {
    assert.equal(candidate.segments[index].id, raw.segments[index].id);
    assert.equal(candidate.segments[index].start, raw.segments[index].start);
